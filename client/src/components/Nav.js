...
const Nav = () => {
    ...
    const { user, filter } = useSelector(state => ({ ...state }))
    ...
    return (
        <div className="bg-white relative  xxs:px-8 px-2 py-3 rounded-[4px] border-b-secondary border-b-[4px]">
            {
                userMenuShow && <div className="flex absolute xxs:right-8 right-0 bg-[#3d5a80] text-white z-50 top-[88%] space-y-2 py-1 flex-col w-full xxs:w-[200px] px-2 font-montserrat text-sm rounded-sm">
                    ...
                    {(user && user.token) &&
                        <Link
                            ...
                            to={user.role === 'admin' ? '/admin/dashboard' : '/user/history'} className='nav-link'><DashboardOutlined />
                            <span>
                                Dashboard
                            </span>
                        </Link>
            ...
            <ul className="flex justify-start py-0 items-center m-0 relative">
                ...
                >
                    <img
                        ...
                        className="h-11 w-auto md:h-12 md:w-auto"
                        alt="Logo"
                ...
                <li>
                    <FaUserCircle className={`text-primary hover:text-secondary cursor-pointer absolute right-0 top-[50%] translate-y-[-50%] text-3xl justify-center ${!userMenuShow ? 'text-primary' : 'text-secondary'}`}
